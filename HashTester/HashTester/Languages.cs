...
namespace HashTester
{
    public static class Languages
    {
        ...
        public static string Translate(int id)
        {
            ...
            if (englishDictionary == null) LoadEnglishLookup();
        ...
        {
            if (englishLookup == null) LoadEnglishLookup();
            if (englishLookup.TryGetValue(englishText.Trim(), out int id)) return Translate(id);
        ...
        public enum L
        {
            ...
            smallUpdate = 10043,
            dontRemindMe = 10044,
            newVersionAppAvailable = 10045,
            newVersion = 10046,
            currentVersion = 10047,
            errorCheckVersion = 10048,
            remindMe = 10049,
        ...
        public static bool LoadDictionary(string nameOfLanguage)
        {
            ...
            {
                MessageBox.Show("Missing Translation for: " + nameOfLanguage + "." + Environment.NewLine + ex.Message, Languages.Translate(L.Error), MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
        ...
        private static string GetPath(string name)
        {
            ...
            name.ToLower() + ".txt",
            name.ToUpper() + ".txt",
            char.ToUpper(name[0]) + name.Substring(1).ToLower() + ".txt"
        };
            foreach (string file in temp)
                foreach (string variant in possibleNames)
                    if (Path.GetFileName(file) == variant) return Path.GetFullPath(file);
            return null;
        }
    }
